"""T4 — does the cost filter survive one target step away?

A ROBUSTNESS CHECK on T2, registered as one so it cannot later be reported
as a search for a better number.

cost_R = fixed per-contract cost / per-contract risk. Both terms come from
the opening-range height, so the filter is a property of the SETUP and
selects the SAME TRADES at any target. Only the outcome distribution
differs. If cost filtering is real it must work at target_r=1.0 as it did at
2.0; if it works only where it was found, it is an interaction or noise.

Second reason for 1.0: T1 measured sd rising 0.79 -> 1.71 across the target
ladder, and the floor scales with sd. This attacks the floor through
VARIANCE rather than sample size -- the only free lever left.

Scored under the MEASURED intra-cluster correlation (0.3818), not the
asserted 0.9 that every earlier power calculation used.
...
TARGET_R, RISK, STOP_RANGE, SEED = 1.0, 175.0, 0.5, 20260830
CLUSTER_SIZE, INTRA_R = 2, 0.3818   # MEASURED, not asserted
INSTRUMENTS = ("MES", "MNQ")
...
def main() -> int:
    ...
    for thr in LADDER:
        ...
        res = Result(name=f"T4 cost_R<={thr} - net R per trade",
